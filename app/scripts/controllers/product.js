			...
			function ($rootScope, $scope, vnApi, $location, $routeParams, $filter, $anchorScroll, $translate, vnCart, vnAppMessageService, vnProduct, snapRemote, notifications) {
				...
				vnApi.Product().get({slug: $routeParams.slug}).$promise
						...
						.then(function () {
							...
							// accessories
							vnApi.Product().get({
								accessoriesOf: $scope.product.code,
								pageNumber   : 1,
								pageSize     : 4
							}).$promise
									.then(function (response) {
				...
				$scope.addToCart = function () {
					...
					if (vnProduct.findRequiredOptionsAreSelected().length > 0 || !vnProduct.findOptionAvailability($scope.product.optionSelection.key)) {
